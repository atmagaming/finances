...
async function fetchSensitiveData(): Promise<SensitiveData[]> {
  ...
  return pages.map((p) => {
    ...
    return {
      ...
      monthlyPaid: formulaNumber(p.properties["Monthly Paid"]),
      monthlyInvested: formulaNumber(p.properties["Monthly Invested"]),
      monthlyTotal: formulaNumber(p.properties["Monthly Total"]),
