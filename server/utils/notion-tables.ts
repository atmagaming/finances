...
const client = new Client({ auth: config.notionApiKey as string });

export const peopleTable = new NotionTable({
  client,
  dataSourceId: config.notionPeopleDbId as string,
  properties: {
    Name: { type: "title" },
...
export const sensitiveDataTable = new NotionTable({
  client,
  dataSourceId: config.notionSensitiveDataDbId as string,
  properties: {
    Name: { type: "title" },
...
export const payeesTable = new NotionTable({
  client,
  dataSourceId: config.notionPayeesDbId as string,
  properties: {
    Name: { type: "title" },
    Person: { type: "relation" },
...
export const transactionsTable = new NotionTable({
  client,
  dataSourceId: config.notionTransactionsDbId as string,
  properties: {
    Note: { type: "title" },
    Amount: { type: "number" },
...
export const vacationsTable = new NotionTable({
  client,
  dataSourceId: config.notionVacationsDbId as string,
  properties: {
    Person: { type: "relation" },
...
// The ORM returns raw Notion objects for formula/relation/people properties
// instead of clean values. These helpers extract the actual values.
